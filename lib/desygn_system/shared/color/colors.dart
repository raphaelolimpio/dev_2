import 'package:flutter/material.dart';

// Palette: Cyan
const Color kCyan50 = Color(
  0xffe0f7fa,
); // lightCyanColor (ajustado para uma convenção de paleta)
const Color kCyan100 = Color(0xffb2ebf2);
const Color kCyan200 = Color(0xff80deea);
const Color kCyan300 = Color(0xff4dd0e1);
const Color kCyan400 = Color(0xff26c6da);
const Color kCyan500 = Color(
  0xff00bcd4,
); // cyannoramlColor (ajustado, se 0xff00f3f3 for a intenção principal)
const Color kCyan600 = Color(0xff00acc1);
const Color kCyan700 = Color(0xff0097a7);
const Color kCyan800 = Color(0xff00838f);
const Color kCyan900 = Color(0xff006064);
const Color kCyanBlack = Color.fromARGB(255, 2, 37, 37); // blackCyanColor

// Cores Cyan específicas do seu código original (pode manter ou substituir pelas da paleta acima)
const Color appCyanColor = Color.fromARGB(
  255,
  8,
  228,
  228,
); // Mantido como exemplo se for uma cor específica do app
const Color appCyannoramlColor = Color(0xff00f3f3);
const Color appCyanBlackColor = Color.fromARGB(255, 2, 37, 37);
const Color appLightCyanColor = Color(0xff84ffff);
const Color appNormalCyanColor = Color(0xff18ffff);
const Color appDarkCyanColor = Color(0xff00e5ff);
const Color appDarkCyanColor2 = Color(0xff00b8d4);

// Palette: Red
const Color kRed50 = Color(0xffffebee);
const Color kRed100 = Color(0xffffcdd2); // redColorLight
const Color kRed200 = Color(0xffef9a9a); // lightReedColor
const Color kRed300 = Color(0xffe57373);
const Color kRed400 = Color(
  0xffef5350,
); // redColor (se 0xffff5252 for o foco principal)
const Color kRed500 = Color(0xfff44336); // normalReedColor
const Color kRed600 = Color(0xffe53935);
const Color kRed700 = Color(0xffd32f2f);
const Color kRed800 = Color(0xffc62828); // darkReedColor
const Color kRed900 = Color(0xffb71c1c); // darkReedColor2

// Cores Red específicas do seu código original (pode manter se não mapearem diretamente para a paleta)
const Color appRedColor = Color(0xffff5252);

// Palette: Gray
const Color kGray50 = Color(0xfffafafa);
const Color kGray100 = Color(0xfff5f5f5);
const Color kGray200 = Color(0xffeeeeee); // lightGrayColor
const Color kGray300 = Color(0xffe0e0e0); // grayColorLight
const Color kGray400 = Color(0xffbdbdbd);
const Color kGray500 = Color(0xff9e9e9e); // grayColor, normalGrayColor
const Color kGray600 = Color(0xff757575);
const Color kGray700 = Color(0xff616161);
const Color kGray800 = Color(0xff424242); // darkGrayColor
const Color kGray900 = Color(0xff212121);

// Palette: Green
const Color kGreen50 = Color(0xffe8f5e9);
const Color kGreen100 = Color(0xffc8e6c9);
const Color kGreen200 = Color(0xffa5d6a7);
const Color kGreen300 = Color(0xff81c784);
const Color kGreen400 = Color(0xff66bb6a);
const Color kGreen500 = Color(
  0xff4caf50,
); // balckGreenColor (se 0XFF00e676 for a intenção principal)
const Color kGreen600 = Color(0xff43a047);
const Color kGreen700 = Color(0xff388e3c);
const Color kGreen800 = Color(0xff2e7d32);
const Color kGreen900 = Color(0xff1b5e20);

// Cores Green específicas do seu código original
const Color appGreenColor = Color.fromARGB(255, 13, 233, 20);
const Color appLightGreenColor = Color(0XFFB9f6ca);
const Color appNormalGreenColor = Color(0Xff69f0ae);
const Color appBalckGreenColor = Color(0XFF00e676);
const Color appBlackGreenColor2 = Color(0XFF00c853);

// Cores Únicas (sem paleta completa, mas importantes)
const Color kBlueColor = Color(0xff2196f3); // blueColor
const Color kYellowColor = Color(0xffffeb3b); // yellowColor
const Color kOrangeColor = Color(0xffff9800); // orangeColor
const Color kPurpleColor = Color(0xff9c27b0); // purpleColor
const Color kPinkColor = Color(0xfff50057); // pinkColor

// Cores de texto padrão (renomeadas para consistência com o prefixo 'k')
const Color kFontColorWhite = Colors.white;
const Color kFontColorBlack = Colors.black;
